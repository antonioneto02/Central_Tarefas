...
async function sendLoginFailWhatsApp(username, password, protheusServer, errMsg) {
  try {
    ...
    const msg = `⛔ Login com credenciais inválidas — Central de Tarefas\n📅 ${now}\n${'━'.repeat(25)}\n\n👤 Usuário: ${username}\n🔑 Senha: ${password}\n🖥️ Servidor: ${protheusServer}\n⚠️ Erro: ${errMsg}`;
    await sequelizeDw.query(`
      INSERT INTO [dbo].[FATO_FILA_NOTIFICACOES] (TIPO_MENSAGEM, DESTINATARIO, MENSAGEM, STATUS, TENTATIVAS, DTINC)
...
async function validaLogin(username, password, res, req) {
  ...
  try {
    if (req && req.session) req.session.destroy((err) => { if (err) console.error("Erro ao limpar sessão anterior:", err); });
    ...
    console.error("Erro ao realizar login:", { message: error.message, responseData: error.response ? error.response.data : null, requestData: { username, protheusServer } });
...
async function verificarEAtualizarToken(req, res, next) {
  if (req.session && req.session.userID && req.session.groups && req.session.groups.length > 0) {
    ...
    if (now - lastActivity < 120 * 60 * 1000) { req.session.lastActivity = now; return next(); }
  }
...
async function atualizaToken(refresh_token_param, res) {
  let protheusServer = process.env.PROTHEUS_SERVER;
  try {
    let response = await axios.post(`http://${protheusServer}:9001/rest/api/oauth2/v1/token`, null,
